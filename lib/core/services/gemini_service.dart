...
class GeminiService {
  ...
  GeminiService() {
    _model = GenerativeModel(
      model: 'gemini-3-flash-preview', // Fastest stable model
    ...
    try {
      ...
      parts.add(
        TextPart(
          ...
          "\n\n"
          "CRITICAL INSTRUCTION: MATH FORMATTING\n"
          "1. MIXED TEXT & MATH: When explaining steps, WRAP ALL MATH in '\$' signs. e.g. 'Calculate \$2x + 5\$'.\n"
          "2. PURE MATH: If the line is just an equation, ALSO WRAP IT in '\$'. e.g. '\$x^2 + 5x + 6 = 0\$'.\n"
          "3. LATEX: Always use LaTeX symbols inside '\$'. e.g. '\$\\sqrt{x}\$' instead of 'sqrt(x)'.\n"
          "\n\n"
          ...
          "CRITICAL: You MUST identify the Form and Chapter ID for EVERY weakness. Use your internal knowledge of KSSM. e.g. 'Linear Equations' is Form 1 Chapter 6.\n"
          ...
          "  \"syllabus_matches\": [{\"form\": <int, 1-5>, \"chapter\": <int, 1-20>, \"confidence\": <float 0.0-1.0>}], // TOP-LEVEL: Identify the Main Chapter(s)\n"
          "  \"weaknesses\": ["
          "    {"
          ...
          "      \"mistake_instances\": ["
          "        {"
          "           \"mistake\": \"<FORMAT: '[Tag]: \\\\n [Previous Step] \\\\n -> [Student Error]'. 1. TAG: Max 5 words. Use words ONLY if difficult to explain with math. 2. PREVIOUS STEP: The valid line right before the error. IF it is the first line, use the RAW QUESTION EXPRESSION (e.g. '\$3+4(n-1)\$') as [Previous Step]. 3. STUDENT ERROR: The exact raw expression from the image. CRITICAL: Wrap ALL math in '\$'. EXAMPLE: 'Expansion Error: \\\\n \$2(x+3)\$ \\\\n -> \$2x+3\$'.>\", \n"
          "           \"correction\": \"<FORMAT: '[Previous Step] \\\\n -> [Correct Step]'. Show the derivation from the SAME prev step. CRITICAL: Wrap ALL math in '\$'. EXAMPLE: '\$2(x+3)\$ \\\\n -> \$2x + 6\$'.>\",\n"
          ...
          "      \"syllabus_refs\": [{\"form\": <int, 1-5>, \"chapter\": <int, 1-20>, \"confidence\": <float 0.0-1.0>}] // Link weakness to specific subtopic if distinct\n"
      ...
      try {
        ...
        if (e.toString().contains('503')) {
          debugPrint(
            "Gemini 3 Flash overloaded, switching to Gemini 2.0 Flash...",
          );
